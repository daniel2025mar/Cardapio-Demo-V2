...
// ==========================================
// CARROSSEL DAS CATEGORIAS + FUNÇÃO DE SEÇÃO
// ==========================================

// Elementos
const scrollArea = document.getElementById("catScroll");
...
// Largura dos cards por dispositivo
function larguraCard() {
  if (window.innerWidth <= 768) {
    return 140; // 3 por tela
  } else {
    return 160; // 5 por tela alinhado
  }
}

// Rolagem ao clicar nas setas
function scrollRight() {
  scrollArea.scrollBy({
    left: larguraCard() * 1.2,
    behavior: "smooth"
  });
}
...
function atualizarSetas() {
  ...
  btnLeft.style.opacity = scrollArea.scrollLeft <= 10 ? "0.3" : "1";
  btnRight.style.opacity = scrollArea.scrollLeft >= maxScroll - 10 ? "0.3" : "1";
}

scrollArea.addEventListener("scroll", atualizarSetas);
...
// Todos os produtos
const produtoscate = document.querySelectorAll("[data-produto]");

// Todas as seções (sem esconder seções inteiras)
const secoes = document.querySelectorAll("[data-section]");
...
categoriaBtns.forEach(btn => {
  btn.addEventListener("click", (e) => {
    ...
    // Mostrar sempre todas as seções
    secoes.forEach(sec => sec.style.display = "block");

    // Filtrar individualmente os produtos
    produtoscate.forEach(prod => {
      const categoriaProduto = prod.getAttribute("data-produto");
