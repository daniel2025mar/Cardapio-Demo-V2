...
// Função para normalizar texto (ignora acentos e caracteres especiais)
function normalizeText(text) {
  return text
    .toLowerCase()
    .normalize("NFD")         // separa os caracteres acentuados
    .replace(/[\u0300-\u036f]/g, "") // remove os acentos
    .replace(/[^\w\s]/gi, ""); // remove caracteres especiais, se quiser
}

// Seleciona o input de pesquisa
const searchInput = document.getElementById("searchInput");

// Seleciona todos os produtos dentro do menu (divs que são itens)
const menuItems = document.querySelectorAll("#menu main > div");

// Seleciona a mensagem de "nenhum resultado" (adicione essa div abaixo do input)
const noResults = document.getElementById("noResults");

searchInput.addEventListener("input", () => {
  const filter = normalizeText(searchInput.value); // texto digitado normalizado
  let hasVisible = false;

  menuItems.forEach(item => {
    const productName = normalizeText(item.querySelector("p.font-bold").textContent);
